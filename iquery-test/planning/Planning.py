...
"""
# 代码解释
1. **add_task_decomposition_prompt**: 当开启专家模式时，为用户问题添加任务拆解的Few-shot示例，帮助模型更好地理解任务分解逻辑。
2. **modify_prompt**: 根据开发者模式设置，动态添加或移除COT（链式思维）和Markdown输出提示词。
3. **get_first_response**: 调用大模型获取响应，处理Rate Limit错误，并在专家模式下进行任务拆解。
4. **function_to_call**: 执行外部函数调用，捕获运行时错误并返回结果。
5. **one_chat_response**: 完整对话流程管理，根据模型返回结果类型调用文本或代码处理函数。
6. **handle_code_response**: 处理代码类响应，支持开发者模式下的代码审查与修改。
7. **check_function_response**: 检查外部函数运行结果，执行高效或深度Debug流程。
8. **handle_text_response**: 处理文本类响应，支持开发者模式下的结果审查与修改。

# 控制流图
```mermaid
flowchart TD
    A[开始] --> B{是否开发者模式?}
    B -->|是| C[modify_prompt 添加提示词]
    B -->|否| D[跳过提示词添加]
    C --> E[继续流程]
    D --> E
    E --> F{是否专家模式?}
    F -->|是| G[add_task_decomposition_prompt 添加任务拆解]
    F -->|否| H[跳过任务拆解]
    G --> I[调用模型获取响应]
    H --> I
    I --> J{是否代码响应?}
    J -->|是| K[handle_code_response 处理代码响应]
    J -->|否| L[handle_text_response 处理文本响应]
    K --> M[检查函数运行结果]
    M --> N{是否报错?}
    N -->|是| O[执行Debug流程]
    N -->|否| P[保存结果]
